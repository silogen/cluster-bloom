...
- **Default**: `""` (empty)
- **Description**: Selects which AIM model sources cluster-forge installs, by hardware family. When left empty, bloom auto-detects the AMD GPU family/families physically present on this node (via a local PCI scan) and uses that as the list; if none is detected (or detection itself fails), it falls back to the legacy branch: only the fixed generic `amd-aim-release-*` + `amd-aim-instinct-*` catalog installs, unchanged from before per-family support existed. `cpu`/`epyc` are never auto-detected — they stay opt-in only. When set explicitly, only the listed families' sources are installed (the legacy generic sources are not installed), and detection never overrides your value.
