...
FILES_TO_COPY="$FILES_TO_COPY $BLOOM_CONFIG"
...
echo "Running: sudo ./$BLOOM_BINARY_NAME cli $BLOOM_CONFIG"
ssh -i "$VM_NAME/qemu-login" -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p 2222 ubuntu@localhost sudo ./$BLOOM_BINARY_NAME cli $(basename "$BLOOM_CONFIG")
