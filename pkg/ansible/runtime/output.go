...
func (p *OutputProcessor) PrintSummary() {
	...
	if p.config != nil {
		...
		if clusterforgeRelease != "" && clusterforgeRelease != "none" && domain != "" {
			...
			fmt.Println()
			fmt.Println("  kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=kgateway -n kgateway-system --timeout=600s && \\")
			fmt.Println("  kubectl wait --for=condition=complete job -l app.kubernetes.io/name=cluster-auth -n cluster-auth --timeout=600s && \\")
